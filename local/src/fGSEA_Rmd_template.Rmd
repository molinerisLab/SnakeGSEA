...
barplot_multi_fGSEA <- function(fgseaRes, qTh = NULL, topPathways = NULL, topN = 20){
  ...
  ggplot(toplot, aes(x = pathway, y = NES, fill = direction, alpha = I(alpha_pval))) + 
    geom_col(width = .8) + coord_flip() + ggtitle(title) + theme_bw() + theme(legend.position = "bottom"
		...
		, plot.title = element_text(size = 8, hjust = 0.5, face = "plain")) + scale_fill_manual(values = c("#004C99", "#CC0000")) + theme(legend.position="none") 
}
```
